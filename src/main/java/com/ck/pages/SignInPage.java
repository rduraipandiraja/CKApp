...
public class SignInPage  extends WrapperMethods{ 
	...
	public SignInPage(AppiumDriver<MobileElement> driver, ExtentTest testInfo) {
		...
		if(Integer.parseInt(apiLevel)>22) {
			...
			try {
				...
				reportStep("About to click on the Allow on the permission popup in SignInPage for the first time", "INFO");
				...
				System.out.println("No Pemission popup comes in the sign page");
				reportStep("No Pemission popup comes in the sign page", "INFO");
	...
	@FindBy(how = How.XPATH, using = "(//android.widget.ImageView)[2]") 
	MobileElement signInOrYouWontGetCashbackImage;
	...
	@FindBy(how = How.XPATH, using = "//android.widget.TextView[contains(@text,'New to CashKaro?')]")
	MobileElement newToCashkaroJoinFreeText;
	...
	public SignInPage validateSignInPage() {
		...
		validateTheElementPresence(signInOrYouWontGetCashbackImage);
		...
		validateTheElementPresence(newToCashkaroJoinFreeText);
	...
	public IntermediateRetailerPage clickSignInButtoForRetailerNavigation(){
	...
	public SignInPage validatePasswordFieldErrorMessage() {
		...
		/*testInfo.log(LogStatus.INFO, "Successfully Validated the error message on the user name field - 1 ");*/
	...
	public SeeVideoToEarnCashbackPage clickOnBackButton() {
		...
		return new SeeVideoToEarnCashbackPage(driver, testInfo);
	}

	public SeeVideoToEarnCashbackPage clickOnBackButton_SecondTimeNavigationToTheGetStartedPage() {
		...
		return new SeeVideoToEarnCashbackPage(driver, testInfo,0);
	}
	...
	public SignInPage enterNumericIntoUserNameField() {
		...
		reportStep("Entering the numberic characters into the UserName field to verify the error message", "INFO");
